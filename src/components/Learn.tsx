...
export const Learn: React.FC<LearnProps> = ({ stageIndex, onBackToCurriculum, onNavigate }) => {
  ...
  // Persists the lesson attempt when the user successfully finishes the lesson buffer
  useEffect(() => {
    if (isCompleted && startTime && !finalStats) {
      const endTime = Date.now();
      const durationSeconds = Math.max(1, Math.round((endTime - startTime) / 1000));
      
      // Compute standard WPM (Words Per Minute) where 1 word = 5 keystrokes
  ...
  }, [isCompleted, startTime, errorsCount, activeLesson.id, finalStats, targetText.length]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (pressed === ' ' && targetChar === ' ') {
        ...
        // Exclude system keys, modifier releases, or custom controls from incrementing errors
